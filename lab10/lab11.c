...
#define MAX_FILE_SIZE 512
  ...
  size_t msgLen = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  size_t sigLen = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  EVP_MD_CTX *mdCTX = EVP_MD_CTX_new();
  if (EVP_DigestVerifyInit(mdCTX, NULL, EVP_sha256(), NULL, pubkey) != 1) {
    perror("EVP_DigestVerifyInit");
    return -1;
  }

  if (EVP_DigestVerifyUpdate(mdCTX, message, msgLen) != 1) {
    perror("EVP_DigestVerifyUpdate");
    return -1;
  }
