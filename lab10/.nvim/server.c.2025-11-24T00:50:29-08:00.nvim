...
Answer: the client is sending data to the server via the write function, using
IPV4 internet protocols (AF_INET)
2. What data is the client sending to the server?
Answer: the client is sending strings encapsulated in a buffer of size 1024
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
Answer: run_acceptor is passed a pointer to a acceptor_args struct, containing a
boolean variable that tells the thread when to stop accepting connections, an
int that contains the file descriptor of the client, a pointer to a list_handle
struct where the message is stored, and a pointer to a mutex used for the
protecting access to the linked list
2. How are received messages stored?
Answer: messages are being stored in a linked list
3. What does `main()` do with the received messages?
Answer: main checks the list count for the total number of messages, freeing all
messages at the end.
4. How are threads used in this sample code?
Answer: each client is given its own thread by the acceptor thread which
processes and adds messages to the list. the acceptor thread runs until enough
messages are received


How are sockets made non-blocking?
answer: sockets are made non blocking using fncntl and setting the O_NONBLOCK
flag What sockets are made non-blocking? answer: sfd inside run_acceptor, cfd
inside run_client Why are these sockets made non-blocking? What purpose does it
serve? answer: run_acceptor is non blocking so it can check for when to stop
run_client is non blocking to prevent the thread from hanging if the client
stops sending data
*/
#include <arpa/inet.h>
#include <errno.h>
...
int main() {
  ...
  while (count < MAX_CLIENTS * NUM_MSG_PER_CLIENT) {
